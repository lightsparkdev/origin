...
function DrawerDemo() {
  const [selected, setSelected] = React.useState<typeof drawerRequests[0] | null>(null);

  return (
    <div style={{ marginBottom: '128px' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 0 }}>
        <thead>
          <tr style={{ borderBottom: 'var(--stroke-xs) solid var(--border-primary)' }}>
            <th className="label-sm" style={{ textAlign: 'left', padding: 'var(--spacing-xs) var(--spacing-sm)', color: 'var(--text-tertiary)' }}>Method</th>
            <th className="label-sm" style={{ textAlign: 'left', padding: 'var(--spacing-xs) var(--spacing-sm)', color: 'var(--text-tertiary)' }}>Path</th>
            <th className="label-sm" style={{ textAlign: 'right', padding: 'var(--spacing-xs) var(--spacing-sm)', color: 'var(--text-tertiary)' }}>Status</th>
            <th className="label-sm" style={{ textAlign: 'right', padding: 'var(--spacing-xs) var(--spacing-sm)', color: 'var(--text-tertiary)' }}>Duration</th>
          </tr>
        </thead>
        <tbody>
          {drawerRequests.map((req) => (
            <tr
              ...
              style={{ borderBottom: 'var(--stroke-xs) solid var(--border-primary)', cursor: 'pointer' }}
              onMouseOver={(e) => { e.currentTarget.style.backgroundColor = 'var(--surface-hover)'; }}
              onMouseOut={(e) => { e.currentTarget.style.backgroundColor = ''; }}
            >
              <td className="body-sm" style={{ padding: 'var(--spacing-sm)', color: 'var(--text-primary)', fontFamily: 'var(--font-mono, monospace)' }}>{req.method}</td>
              <td className="body-sm" style={{ padding: 'var(--spacing-sm)', color: 'var(--text-primary)', fontFamily: 'var(--font-mono, monospace)' }}>{req.path}</td>
              <td className="body-sm" style={{ padding: 'var(--spacing-sm)', textAlign: 'right' }}>
                <Badge variant={req.status < 300 ? 'green' : 'red'}>{req.status}</Badge>
              </td>
              <td className="body-sm" style={{ padding: 'var(--spacing-sm)', textAlign: 'right', color: 'var(--text-secondary)' }}>{req.duration}</td>
            </tr>
          ))}
        </tbody>
      </table>

      <Drawer.Root open={!!selected} onOpenChange={(open) => { if (!open) setSelected(null); }} swipeDirection="right">
        <Drawer.Portal>
          <Drawer.Backdrop />
                ...
                <>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 'var(--spacing-md) var(--spacing-xl)', borderBottom: 'var(--stroke-xs) solid var(--border-primary)' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--spacing-xs)' }}>
                      <Badge variant={selected.status < 300 ? 'green' : 'red'}>{selected.status}</Badge>
...
export default function Home() {
  return (
    <main style={{ padding: '2rem', maxWidth: '600px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--spacing-lg)', marginBottom: '128px', maxWidth: 480 }}>
        <Collapsible.Root>
        ...
        <PreviewCard.Root>
          ...
          <PreviewCard.Portal>
            <PreviewCard.Positioner sideOffset={8}>
              <PreviewCard.Popup style={{ padding: 'var(--spacing-md)', width: 280 }}>
                <PreviewCard.Arrow />
                <p style={{ margin: 0 }}>A lightweight preview of the linked content</p>
              </PreviewCard.Popup>
          ...
          <PreviewCard.Portal>
            <PreviewCard.Positioner sideOffset={8}>
              <PreviewCard.Popup style={{ width: 300 }}>
                ...
                <div style={{ padding: 'var(--spacing-md)', display: 'flex', flexDirection: 'column', gap: 'var(--spacing-2xs)' }}>
                  <span className="label" style={{ color: 'var(--text-primary)' }}>Typography Guide</span>
                  <span className="body-sm" style={{ color: 'var(--text-secondary)' }}>Learn about text styles, mixins, and the type scale</span>
                </div>
              </PreviewCard.Popup>
            </PreviewCard.Positioner>
          </PreviewCard.Portal>
